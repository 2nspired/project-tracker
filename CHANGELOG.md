...
- **Costs surface simplified — board scope deferred until card-attribution is automated** (#225). The v6.1.0 board-scoped Costs lens (#200 phases 1a / 2a / 2c / 3) shipped ahead of its prerequisite: automatic card-attribution of Stop-hook events. With every event landing in the DB with `cardId: null`, board-scoped queries deterministically return $0 and the entire board-mode UX renders empty. #223 stopped the false "Set up tracking" CTA from firing on top of that empty state, and #224 made `<BoardPulse>` match by going board-scoped, but consistency-of-zero isn't useful — the user came to a Costs page to see costs, and the surface that actually has them is project-wide. This card defers the board scope until `attributeSession` (#191) runs automatically on Stop-hook fire so events have `cardId` set and board-scoped queries return meaningful data. **Concrete changes:** `<CostsPage>` (`src/components/costs/costs-page.tsx`) ignores `boardId` for data and child rendering — drops the board-scoped queries, the `projectWideSummary` query, the `projectHasAnyData` fallback (#223), the `BOARD'S SHARE` summary cell, the `(project-wide)` scope badges, the cross-board caveat paragraph, and the conditional pricing-table hiding. The container widens from `max-w-3xl` (768px) to `max-w-5xl` (1024px) to match other content surfaces. The board-page Costs link drops `&board=` (keeps `?from=` for the breadcrumb back-link). `<BoardPulse>` (`src/components/board/board-pulse.tsx`) reverts to project-wide cost queries — removes the `projectWideSummary` query, the attribution-gap branch in the popover, and the `showAttributionGap` rendering — restoring the simple `hasAnyCost` / `showCostSetupCta` form. The `boardId` prop on `<CostsPageProps>` is preserved (route plumbing intact) but unused. Verified live on the launchd service: navigation from Development Board → Costs renders `Lifetime cost $3970 · Last 7 days $3970 · Sessions 40 · Tracking since 2 days ago` — real data instead of the previous all-zeros board view. 306 tests pass; lint clean. Closes #225, supersedes the board-mode-specific behavior added in #223 and #224.
