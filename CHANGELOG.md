...
- **`<SegmentedControl>` primitive — single-density (h-8) toggle group for "pick one of N equivalent options"** (#242). New `src/components/ui/segmented-control.tsx` wraps Radix `ToggleGroup` (via the unified `radix-ui` package) for built-in roving-tabindex keyboard nav and `aria-pressed` semantics; the named export is `<SegmentedControl>` because that's the term designers reach for first (Apple HIG, GitHub Primer). Two corner-radius variants — `shape="md"` (default, connected pill with hairline border around the group) for "single-select among equals" surfaces (view toggles, period selectors, density toggles) and `shape="full"` for filter chips where the data tag is round (each item its own capsule, no outer border). Two sizes — `size="default"` (h-8 px-2.5) and `size="icon"` (h-8 w-8). Showcase page at `/dev/design/primitives/segmented-control` renders five live variants with copyable usage code; nav entry promoted from "Coming soon" placeholder to "Live". Closes audit findings C3 + C14.
...
- **Six ad-hoc toggle/pill/chip implementations migrated to `<SegmentedControl>` for visual density consistency** (#242). Six surfaces previously rolled their own `aria-pressed` button rows with bespoke styling; this card converts them to the new primitive: (1) `src/app/(main)/projects/[projectId]/boards/[boardId]/page.tsx` board kanban/list view toggle (was h-8 ad-hoc Button pair with `rounded-r-none` / `rounded-l-none`), (2) `src/components/notes/note-views.tsx` `NoteViewToggle` (same pattern, h-8 icon Buttons), (3) `src/components/roadmap/roadmap-view.tsx` density toggle (was a different density at h-6 — now consolidated to h-8 to match the rest), (4) `src/components/costs/section.tsx` `PeriodPills` (was rounded-full uppercase pill; now connected `rounded-md` group), (5) `src/components/costs/card-delivery-section.tsx` local `PeriodPills` (was a parallel implementation at rounded-md; now imports the shared `PeriodPills` + `Period` type from `costs/section.tsx`, removing the duplicate component and `PERIOD_OPTIONS` table), (6) `src/components/board/session-shell.tsx` `FilterChip` plus the duplicate copy inline in `src/components/board/activity-sheet.tsx` (rounded-full no-border chip). The `FilterChip` export is removed from `session-shell.tsx`; consumers (`activity-sheet.tsx`, `handoffs-sheet.tsx`) now use `<SegmentedControl shape="full">` directly. The handoffs-sheet agent filter (where "no agent" === "all agents") is modeled with a `__all__` sentinel value mapped back to `null` at the handler boundary. The Badge-as-chip pattern in `notes/note-views.tsx` `NoteTagFilter` (line 91) is left as-is — those tags are independent multi-select toggles where each tag's identity matters more than the chosen-vs-unchosen ratio, so the round Badge presentation reads cleaner than a SegmentedControl group. Closes #242.
- **Loading states migrated to named primitives — raw `animate-pulse` outside the `<Skeleton>` primitive eliminated, plain-text "Loading…" strings replaced with `<LoadingRow>`** (#244). Audit findings C5 + C6 from the v6.2 design audit. The three grandfathered `raw-animate-pulse` baseline entries from #255 (`src/components/board/handoffs-sheet.tsx:357-359`) are gone — `HandoffsSkeleton` now renders three `<RowSkeleton>` instances. Four plain-text loading states migrated: `<PricingOverrideTable>` (`Loading pricing…`) and `<McpCatalogPopover>` (`Loading tools…`) now render `<LoadingRow text=…>`; `<ActivitySheet>` (`Loading…`) renders the default `<LoadingRow>`; `<CommandPalette>` empty state renders `Searching…` (single-character ellipsis `…` instead of `...`). Board-page loading skeleton (`src/app/(main)/projects/[projectId]/boards/[boardId]/page.tsx:250`) widened from `w-72` to `w-84` so it matches the rendered `<BoardColumn>` width and the layout doesn't pop +48px wider when data lands — the headline geometry mismatch from the audit. Named skeleton primitives `<CardSkeleton>`, `<RowSkeleton>`, `<HeadingSkeleton>`, plus `<LoadingRow>` added to `src/components/ui/skeleton.tsx`. Showcase page at `/dev/design/patterns/loading-empty-error` replaces the `#237` `<ComingSoon />` placeholder with live demos. Closes #244.
- **Tooltip delay standardized to 500ms across the app + `(main)` header extracted into `<TopNav />` + Cmd-K palette open-state lifted into a context hook** (#243). Two paired design fixes from the v6.2 audit (C20 + C21). **Tooltip delay:** the root `<TooltipProvider>` in `src/app/layout.tsx` left `delayDuration` at the component default of `0` (set in `src/components/ui/tooltip.tsx`), so hovering anything fired a tooltip on the first frame — jarring vs the Linear / GitHub ~400–500ms convention. Bumped the root provider to `delayDuration={500}` and dropped four redundant per-page `<TooltipProvider>` wrappers (`src/app/(main)/projects/page.tsx`, `src/app/(main)/projects/[projectId]/boards/[boardId]/page.tsx`, `src/app/(main)/dashboard/page.tsx`) that re-wrapped without an override and silently reset the delay back to `0`. Two intentional `delayDuration={200}` overrides on `<TagManager>` and `<MilestoneManager>` are preserved — those carry density-specific UX intent for tag/milestone chips. **TopNav extraction:** `src/app/(main)/layout.tsx` carried 100 lines of inline header JSX (brand link, primary nav, search button, MCP catalog trigger, docs link, server-status pill, theme toggle); extracted to `src/components/layout/top-nav.tsx` so the layout file is now 14 lines and the header is independently grep-able + testable. **Cmd-K refactor:** the search button previously faked a Cmd-K keystroke via `document.dispatchEvent(new KeyboardEvent("keydown", { key: "k", metaKey: true }))` — fragile (depends on event-bubble ordering, can be swallowed by other listeners, impossible to type-check) and a smell (a button manufacturing a synthetic keyboard event to talk to a sibling component). `src/components/command-palette.tsx` now exposes `<CommandPaletteProvider>` (owns open-state + the global Cmd-K listener) and `useCommandPalette()` (returns `{ open, setOpen, toggle }`); `(main)/layout.tsx` wraps its subtree in the provider and `<TopNav>`'s search button calls `setOpen(true)` directly. The off-token `text-[10px]` on the search button's `<kbd>` moves with the file and is grandfathered in `scripts/design-lint-baseline.json` for #240 to clean up. Closes #243.
- **Migrated 15 hardcoded `text-[10px]` / `text-[0.625rem]` sites to the `text-2xs` token** (#240). Audit finding (design C7) — same regression class as #222, where the v4 token rename silently rendered 133 sites at 16px. 15 sites bypassed the type scale via Tailwind arbitrary-value font sizes (`text-[10px]` and `text-[0.625rem]`) instead of consuming `--text-2xs` (0.6875rem / 11px), so any future tweak to the small-metadata size would miss those sites. Per Linear's accessibility floor of 11px, all 15 sites consolidate onto `text-2xs` rather than introducing a new `--text-3xs: 0.625rem` token tier (Option B from the card plan). Touched: `src/components/ui/{token-cost-chip,pigeon-overhead-chip,actor-chip}.tsx`, `src/components/board/{board-card,board-list-view,card-detail-sheet,activity-sheet}.tsx`, `src/components/layout/server-status-pill.tsx`, `src/components/header/{mcp-catalog-trigger,mcp-catalog-popover}.tsx`, `src/components/design-system/sidebar.tsx`, `src/components/layout/top-nav.tsx` (the `<kbd>⌘K</kbd>` from #243's TopNav extract). The `lint:design` CI gate (#255) baseline now drops from 8 grandfathered `arbitrary-text-size` entries to 1 — only the unrelated `text-[8px]` in `src/components/roadmap/progress-ring.tsx` remains. Acceptance: zero `text-\[\d+px\]` / `text-\[0\.\d+rem\]` matches in `src/` for the targeted variants. Closes #240.
...
- **Named skeleton primitives (`<CardSkeleton>` / `<RowSkeleton>` / `<HeadingSkeleton>`) + `<LoadingRow>`** (#244). `src/components/ui/skeleton.tsx` previously exported only the raw `<Skeleton>` div; consumers were inventing per-call-site geometries (11 distinct shapes across ~15 callers per the v6.2 audit) — and several were drifting from the real component (`<BoardColumn>` is `w-84` but the board page skeleton was `w-72`). The named primitives codify the right geometry once: `<CardSkeleton>` matches `<BoardCard>` (`w-84`, three-line title/tags/footer shape), `<RowSkeleton>` matches `<HandoffRow>` / activity-row layout, `<HeadingSkeleton>` matches the H1 + subhead pair used at the top of every page header. `<LoadingRow>` is the inline single-line state for popovers and command-palette empty rows where a shape skeleton would be overkill — it carries `role="status"` + `aria-live="polite"` so the transition is announced, and uses the single-character ellipsis `…` always. Pattern reference: Linear's "loading state matches the real layout shape" docs guideline. Showcased on the new `/dev/design/patterns/loading-empty-error` page (replacing the `<ComingSoon />` placeholder reserved by #237's design-system shell), with each primitive paired against a one-line "use this when…" guide and surfaced in the design-system sidebar's Patterns group. Closes #244.
