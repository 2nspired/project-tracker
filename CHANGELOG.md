...
- Energy & CO₂ estimate on the Costs page (#180). New `Energy` cell in the SummaryStrip showing total kWh with a tooltip surfacing grams CO₂ and a methodology link; new per-row Energy column on the Top Sessions table; new fourth stat on `<HandoffActivitySection>`. Coefficients live in `src/lib/energy-coefficients-defaults.ts` (8 models seeded with per-row sources from de Vries 2023, Luccioni et al. 2024, Strubell et al. 2019); CO₂ uses the IEA 2024 world-average grid intensity of 475 g/kWh. Derived at read time alongside `costUsd` — no schema migration, no backfill script, coefficient changes apply retroactively. New `formatEnergy` / `formatCo2` helpers (`src/lib/format-energy.ts`) handle bucketed magnitudes (`12.3 Wh` / `1.23 kWh` / `1.40 MWh`; `420.0 g CO₂` / `1.23 kg CO₂` / `1.40 t CO₂`). Methodology and override path documented in `docs/ENERGY-METHODOLOGY.md`. Treat numbers as ballpark (~±50%), not invoice-grade. (#180)
...
- Per-board accent color: optional 6-digit hex on `Board`, set via a small color-picker (native `<input type="color">` + Clear button) in the board header next to the editable name. Renders as a 3px left-stripe on each card in the project's boards list and a thin top-strip inside the open board, giving multi-board projects a glance-distinguishable accent without reading the title. Default behavior unchanged when `accentColor === null`. (#291)
- `scripts/backfill-attribution.ts` — historical backfill for v6.0+ `TokenUsageEvent` rows where `cardId IS NULL`. Replays `Activity` move events backwards to reconstruct the In-Progress set at each row's `recordedAt`, then runs the existing `attribute()` engine (#268). `--dry-run` previews the gap delta + per-signal breakdown before any write; `--confidence=medium|high` (default `medium`) keeps weak inferences null. Idempotent — `WHERE cardId IS NULL` re-runs as a no-op. (#270)
- Attribution Engine signals 3 + 4 wired (session-touch + session-commit). New nullable `Activity.sessionId` and `GitLink.sessionId` columns let `buildAttributionSnapshot` find cards the agent touched or committed against in this MCP server lifetime; signals 3 (`session-recent-touch`, medium) and 4 (`session-commit`, medium-low) now fire when single-In-Progress can't decide and an explicit `cardId` wasn't passed. Anchor strategy: `recordFromTranscript` parses the first transcript message timestamp; `recordManual` uses a 4h lookback. Web-side writes leave `sessionId` null so attribution still falls through cleanly via signals 1+2+5 — the documented "uncanny valley" risk is contained by `attribute()`'s confidence ladder (multi-In-Progress short-circuits to unattributed even when 3/4 would otherwise fire). `npm run service:update` runs `prisma db push` automatically. (#272)
- `RELEASES.md` at repo root — human-facing highlights surface complementing the forensic `CHANGELOG.md`. 3–5 bullets per release at headline-level (≤140 chars, user-language, no card or PR refs) so a non-contributor gets a 30-second summary of what shipped. README's "Releases & upgrades" section now links both files with a one-line distinction; the editorial procedure (manual, write to `[Unreleased]` then promote on tag) is documented under a new "Release notes" section in `CONTRIBUTING.md`. Backfilled v6.5.0 as a working example. (#296)
...
- Per-handoff cost chip in the Handoffs sheet was rendering `no-data` (em-dash) for every visible row in production. Two-part fix: (1) `getHandoffCost` was treating `workingOn[0]` as a card UUID, but `workingOn` is documented free text (`#7 auth` style) — it now extracts the leading `#N` ref and resolves it to a card UUID, falling back to project+agent scope when the ref is absent or doesn't resolve; (2) when single-card narrowing yields zero (events in window lack explicit attribution), it falls back to project+agent scope, and `mapHandoffConfidence` now reserves `no-data` for genuinely empty windows (non-empty events without attribution signals roll up as `estimated`). Smoke verified on Pigeon Dev: 30 visible rows went from 30/30 em-dash to 23/30 estimated chips with real spend. (#297)
