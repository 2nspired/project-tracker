...
- Delete dead-code auth scaffold (`src/utilities/auth/server.ts` + `src/components/auth/auth-provider.tsx`) — leftovers from the Supabase rollback in #231 with no external consumers (#263)
...
- **v6.2 docs sweep — merge `how-it-works.md` into `concepts.mdx`, fix 3 broken archive links, standardize tool count to 65+, patch quickstart/README for v6.1.0** (#248, #249, #250). Three docs cards in one atomic sweep, all targeting the v6.2 consolidation sprint's docs/UX adoption gates. **#250 (canonical-claim conflict + stale dual-bin section):** `docs/how-it-works.md` and `docs-site/src/content/docs/concepts.mdx` covered overlapping ground (sessions, handoffs, briefMe loop, surfaces) with `how-it-works.md:3` claiming "the docs site is canonical" — two docs both claiming canonicality on the same content. Resolution: concepts.mdx wins (it gets the published URL + SEO); `how-it-works.md`'s unique narrative ("The problem" framing, the "Built for two readers" table, "Project policy: tracker.md" section, "MCP surface" prose, "Session loop in one sentence" closer) folded into concepts.mdx; `docs/how-it-works.md` `git rm`'d; README links rerouted to `https://2nspired.github.io/pigeon/concepts/` (Local-first badge link, "long-form design narrative" pointer, Concepts-section list entry — the duplicate "Mental model" entry was removed since concepts.mdx now is the mental model). The stale "dual-bin deprecation calendar" section (~line 110-122) described v5.x as "today" with "dual-bin" support and v6.0 as "the cutover" while we shipped v6.1.0 — replaced with a "Versioning policy" section with v5.x (completed) / v6.0 (completed) / v6.1 (today) / Permanent rows. Also fixed `docs-site/src/content/docs/why.mdx:100` `session_handoff` → `Handoff` (renamed in the Note+Claim cutover, #179 Phase 2). **#249 (broken archive links + tool-count drift):** four files referenced docs that moved to `docs/archive/` in the v6 cleanup — `docs/SURFACES.md:87` (`RFC-WORKFLOW.md` → `archive/RFC-WORKFLOW.md`), `docs-site/src/content/docs/tracker-md.mdx:191` (`MIGRATING-TO-PIGEON.md` → `archive/`), `docs-site/src/content/docs/tools.mdx:122` (`RFC-NOTE-CLAIM-PRIMITIVES.md` → `archive/`); the fourth (`concepts.mdx`'s `MIGRATING-TO-PIGEON.md` link) was eliminated by #250's dual-bin rewrite. Tool count drift across five surfaces (README "60+", `how-it-works.md` "60+", `concepts.mdx` "50+", `why.mdx` "50+", `tools.mdx` auto-syncs to actual ~65) reconciled to a single canonical string **65+** — tool registry currently exposes 65 extended tools per `src/lib/tool-catalog.generated.ts`; auto-syncing the prose mentions was scoped out (would require new marker pairs in three more files for one number; the existing `npm run docs:sync` covers the README essentials/extended tables but not free-prose mentions). **#248 (quickstart + README install-block patches):** quickstart's step 4 didn't mention that `connect.sh` installs slash commands (#147) and the Stop hook (#217) — added a three-bullet "what connect.sh does" breakdown plus an explicit "idempotent — re-run it any time" note so users learn to lean on it; quickstart's "When something goes wrong" already led with `npm run doctor` and linked the new `/troubleshooting` page (post-#247) so no edit needed there; added a "Coming back after an upgrade?" `<Aside>` pointing returning users to the Costs page for the v6.1 token-tracking lens. README install block previously mixed `npm run service:install` (macOS-only) and `npm run dev` (other platforms) into one fenced bash block where a Linux user copy-pasting would hit the macOS-only command — split into a "common setup" block (clone/install/setup) followed by two annotated platform-specific blocks ("macOS" → service:install + doctor, "Linux/Windows/WSL" → dev + doctor in second terminal). Closes #248, #249, #250.
- **`docs/token-tracking.md` slimmed to operator-setup-only — conceptual content consolidated into the docs-site `/costs/` page** (#251). The two docs covered the same conceptual ground (five-column token split, attribution rule, savings formula, conservative framing, pricing decisions, limits), which guarantees drift over time. The audit (#253) flagged `docs-site/src/content/docs/costs.mdx` as the single highest-quality doc in the repo (clear formulas, worked examples, named tradeoffs); this card makes it the canonical conceptual home and reduces `docs/token-tracking.md` to the operator-facing setup guide that the in-app `<TokenTrackingSetupDialog>` footer links to. What stays in `docs/token-tracking.md`: agent coverage matrix (Claude Code automatic / Codex manual / OpenAI no-path), in-app + manual setup walkthroughs, common silent-drop causes (`repoPath` not set, wrong file, script not executable, `mcp_tool` vs `command`), manual `recordTokenUsage` example, privacy note. What moved out (now lives in `/costs/`): the counterfactual methodology section, savings formula derivation, recalibrate-baseline conceptual content, pricing override walkthrough conceptual content, FAQ entries that overlap with the costs page. Cross-links: `token-tracking.md` opens with a "for what these numbers mean, see /costs/" pointer; `/costs/` "See also" updated to describe `token-tracking.md` as operator setup; `src/lib/token-tracking-docs.ts` module comment now describes the deliberate split (operator how-to vs methodology narrative). `TOKEN_TRACKING_DOCS_URL` still points at `docs/token-tracking.md` (the dialog footer wants the operator guide), and `COST_TRACKING_DOCS_URL` still points at `/costs/` — no in-app link relocations needed. Closes #251.
- **`docs/DESIGN-CONTEXT-MODEL.md` archived — schema described no longer exists** (#252). The doc described `PersistentContextEntry`, `CodeFact`, `MeasurementFact`, and a `version Int @default(0)` optimistic-locking pattern across Card / Decision — all four were retired in the Note + Claim consolidation, and the current schema uses `Note` + `Claim` + `Handoff` with last-write-wins (per `AGENTS.md` §Last-Write-Wins). The doc read as a frozen mid-flight RFC that would mislead any agent or contributor who landed on it. Moved to `docs/archive/DESIGN-CONTEXT-MODEL.md` with a one-line preamble pointing at the current model. The single inbound link (in `docs/archive/RFC-NOTE-CLAIM-PRIMITIVES.md`, itself already archived) updated to the new path. Acceptance: no live doc describes a non-existent schema. Closes #252.
- **New docs-site `/troubleshooting` page consolidates scattered failure-mode hints into one stuck-operator landing zone** (#247). Quickstart's inline "Things that sometimes go wrong" section, `docs/UPDATING.md`'s "When something goes wrong" steps, and `npm run doctor`'s output now share a single dedicated page at [`/troubleshooting`](https://2nspired.github.io/pigeon/troubleshooting/) instead of being scattered across three surfaces. Leads with `npm run doctor` as the first-line debug step (it prints copy-pasteable fixes for every failure mode it covers — legacy MCP key shape, hook drift, launchd label drift, missing `.mcp.json` on connected repos, server-vs-package version skew, missing `tracker.md`, WAL-file phantom-drop pressure, FTS5 half-state). Documents nine common failure modes with diagnose + fix steps: MCP server "failed" in Claude Code (worktree vs main, `.mcp.json` missing, legacy `project-tracker` key not renamed); `briefMe` failing on missing `repoPath` (the most common cause in practice); schema drift / mid-update wedge (additive vs destructive `prisma db push`, WAL truncation, backup-restore); FTS5 half-state (orphan shadow tables); macOS launchd silence / wrong label (legacy `com.2nspired.project-tracker` vs new `com.2nspired.pigeon`); stop hook silently no-op'ing for token tracking (wrong file, wrong type, stale `server` field, non-executable script); old MCP tool names returning "tool not found" (points at `docs/MIGRATION-HISTORY.md`); `_versionMismatch` warnings on briefMe (stale MCP client manifest); and a "things were working yesterday" full-reset path. Cross-references the `_upgradeReport` field on briefMe (#215) so users know post-update health surfaces automatically. Wired into three existing entry points: `docs-site/src/content/docs/quickstart.mdx` ("Things that sometimes go wrong" → "When something goes wrong" with a one-line link to the new page), `docs/UPDATING.md` ("When something goes wrong" steps now include `npm run doctor` as step 1 and a link to the troubleshooting page as step 5), and `README.md` (link beneath the doctor section + entry under Documentation → Reference). New sidebar entry under `Reference` between Integration and Commands. Closes #247. (#247)
- **AGENTS.md / AGENT-GUIDE.md split executed — duplicated sections removed from `AGENTS.md`** (#246). PR #164 created `docs/AGENT-GUIDE.md` with the universal "how to use Pigeon" guidance extracted from `AGENTS.md`, but the universal sections were never removed from `AGENTS.md` — leaving hundreds of duplicated lines across two files. This card executes the disposition map at `docs/AGENT-GUIDE-inventory.md`: seven sections (Intent on Writes, Column Definitions, When to Use the Board, Planning a Card, When NOT to Use the Board, What Goes Where, Efficiency Tips) are replaced with one-line pointers to `docs/AGENT-GUIDE.md`. Contributor-only sections — Live tag + milestone API, Project orientation, Tag Conventions, Milestones, Claims/Facts, Commit Summaries, Last-Write-Wins, Decision Supersession, Knowledge Search, Linking Commits to Cards, Connecting to a Project, Token Tracking — stay in `AGENTS.md` as the project-internal reference. The `#97` (Up Next removed) and `#174` (Done sort by ship-date) migration footnotes are kept in `AGENTS.md` as Pigeon-side history. Also fixes a stale `Sessions sheet` → `Handoffs sheet` typo at the Token Tracking section (renamed in #188). Inventory file moved to `docs/archive/AGENT-GUIDE-inventory.md` — the working doc is no longer load-bearing now that the decision is executable. CLAUDE.md / tracker.md / scripts/print-connect-snippet.ts pointers to `AGENTS.md` still resolve — the file still exists, just slimmer (~390 → ~301 lines). Closes #246. (#246)

### Changed

- **Promoted shared `<StepSection>` UI primitive — drops three independent `<Section>` / `<StepLabel>` implementations + the orphaned "01" on Pricing** (#238). Three independent copies of the same step-numbered section frame had grown across the app (`src/components/costs/section.tsx` from #200 Phase 3, a local duplicate inside `src/components/costs/pricing-override-table.tsx`, and a third copy in `src/components/board/token-tracking-setup-dialog.tsx`) — same shape, three slightly different className strings, no shared source of truth. Promoted the canonical implementation to `src/components/ui/step-section.tsx` as `<StepSection>` + `<StepLabel>`, with two opt-in modifiers that absorb the deltas: `flush` (collapses the top border + padding via `first:` Tailwind variants — used by the dialog where the first section sits flush with the header divider; safe to apply uniformly across a stack since only the first child fires) and `tone="anchor"` (preserved from the original costs/section.tsx for future revival of the savings statement). The `step` prop is now optional — when there are no sibling step-numbered sections, a lone `step="01"` reads as orphaned editorial; the Costs page's `<PricingOverrideTable>` is the canonical solo case (post-#225 it's the only numbered section on the page) so this card drops its `step="01"` until Pricing has siblings again. Wired pricing-override-table and token-tracking-setup-dialog to the new primitive; deleted both local duplicates plus the now-orphaned `costs/section.tsx` (no remaining importers — its only callers were the three Costs sub-components removed in #236). `grep -rn "function Section" src/components/` returns zero matches (the unified primitive is `function StepSection`); `grep -rn "function StepSection\\\|function StepLabel" src/components/` returns one match each, both inside `src/components/ui/step-section.tsx`. 284 tests pass; lint + type-check clean. Closes #238.

### Removed

- **Dormant Costs sub-components + their backing tRPC procedures** (#236). Three Costs sub-components were dropped from the rendered Costs page in #225 (board scope + sections-without-data deferred until card-attribution is automated) but kept in the codebase against future revival. They were never re-imported and were drifting out of date alongside CHANGELOG / docs / type changes; their backing procedures had no MCP-tool callers and only the dead component callers + tests, so the entire stack was orphaned. This card removes the components (`src/components/costs/savings-section.tsx`, `pigeon-overhead-section.tsx`, `card-delivery-section.tsx`, plus `savings-methodology-sheet.tsx` which was only imported by the savings lens), the three procedures (`tokenUsage.getSavingsSummary`, `tokenUsage.getPigeonOverhead`, `tokenUsage.getCardDeliveryMetrics`) on `src/server/api/routers/token-usage.ts`, the corresponding service functions + helpers (`getSavingsSummary` / `getPigeonOverhead` / `getCardDeliveryMetrics` / `sumCardCost` / `avgCostForWindow` / `readTokenBaseline` / `savingsCutoff` / `periodCutoff` / `PERIOD_DAYS`, plus the `SavingsPeriod` / `SavingsSummary` / `SavingsSessionEntry` / `PigeonOverheadByTool` / `PigeonOverheadResult` / `PigeonOverheadPeriod` / `CardDeliveryPeriod` / `CardDeliveryEntry` / `CardDeliveryMetrics` exported types) on `src/server/services/token-usage-service.ts`, and the two pure-procedure test files (`token-usage-savings.test.ts`, `token-usage-card-delivery.test.ts`); the `token-usage-pigeon-overhead.test.ts` file had its `getPigeonOverhead` describe block trimmed but the `getSessionPigeonOverhead` describe stays. Kept live: `recalibrateBaseline` (writes `Project.metadata.tokenBaseline`, still callable from MCP), `getSessionPigeonOverhead` (powers `<PigeonOverheadChip>`), and `getCardPigeonOverhead` (powers `<CardPigeonOverheadChip>` on the card detail sheet) — those have non-component callers. `docs/token-tracking.md` Section 04 + the docs-site `costs.mdx` Sections 3-4 carry a "Deferred (#225 / #236)" banner pointing at the disposition; the methodology stays documented for the future revival. Teaching-project's "Track your costs" card description rewritten to drop the four-lens framing and the now-invalid `runTool({ tool: "getSavingsSummary" })` example. 284 tests pass; lint + type-check clean. Closes #236.
...
- **Internal refactor: `src/mcp/extended-tools.ts` (1,643 lines, 25 tools across 8 sections) split into focused per-category modules under `src/mcp/tools/`** (#235). The legacy `extended-tools.ts` was the last surviving monolith from before the `tools/` subdirectory pattern was established (50–200 lines per file). This card finishes the migration: the file is removed entirely; its contents are split along the existing `─── Section ───` dividers into eight focused files — `tools/card-tools.ts` (deleteCard, bulkMoveCards, bulkUpdateCards), `tools/card-create-tools.ts` (bulkCreateCards, createCardFromTemplate), `tools/checklist-tools.ts` (bulkAddChecklistItems, addChecklistItem, toggleChecklistItem), `tools/comment-tools.ts` (listComments), `tools/milestone-tools.ts` (createMilestone, updateMilestone, mergeMilestones, listMilestones), `tools/note-tools.ts` (listNotes, createNote, updateNote), `tools/activity-tools.ts` (listActivity), and `tools/setup-tools.ts` (createProject, createColumn). The Discovery section (listProjects, listBoards, getCard, getStats) and Smart Prioritization section (getWorkNextSuggestion + computeScore helper) merge into the existing `tools/discovery-tools.ts` since they share the `discovery` category. `register-all-tools.ts` updated to import the new paths (single source of truth — `scripts/smoke-workflows.ts` now imports it directly instead of duplicating the per-module list). All eight new files stay under 300 lines each; zero behavior change — same 65 extended tools register with identical names, schemas, categories, and annotations. `npm run docs:check` and `npm run catalog:check` both report "up to date" (no public-surface drift); `npm test` passes (306 tests); MCP server boot smoke confirms `10 essentials + 65 extended`. Closes #235.
...
- **Docs splash + `/api/state` integration example no longer reference the removed "Up Next" column** (#245). Three stale references in `docs-site/src/content/docs/`: the `index.mdx` `<ScreenshotCallout>` `alt` text and column-list prose both named `Up Next` as a live column, and `integration.mdx`'s sample `/api/state` response carried a `"up_next": 0` count with `"schema_version": "1.0"`. The Up Next column was removed in #97 (v3/v4) — the human-priority queue is now expressed by position in Backlog (top 3 = pinned), and `src/server/services/api-state.ts` dropped the `up_next` count in `API_STATE_SCHEMA_VERSION` 1.1 (it is **not** emitted for back-compat). Splash now lists the canonical columns (Backlog, In Progress, Review (optional), Done, Parking Lot) and notes the pinned-top-3 convention; integration example drops the `up_next` field and bumps `schema_version` to `"1.1"` to match the runtime. The board-overview screenshot itself was not re-shot in this card (still shows the old column layout) — surfaced as a follow-up. Closes #245.
