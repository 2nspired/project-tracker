	...
	cards: [
		...
		},
		{
			...
			description: [
				...
				'**Why it matters:** Tags survive column moves. An agent can pull "everything tagged `feature-auth`" across Backlog and In Progress without caring about current status. Normalization eliminates the "232 distinct strings, half singletons" drift the v4.2 rework was scoped to fix.',
				'**Try it (UI):** Open this card and use the tag combobox — type to filter existing tags, or "Create new" if none match.',
				'**Try it (agent — strict, recommended):** `runTool({ tool: "listTags", params: { projectId } })` to discover existing slugs, then `updateCard({ cardId: "#10", tagSlugs: ["tutorial", "organization", "feature-tags"] })`. Strict mode rejects unknown slugs with `_didYouMean` suggestions — call `runTool({ tool: "createTag", params: { projectId, label: "..." } })` first when introducing new vocabulary.',
				'**Try it (agent — legacy, v4.2 only):** `updateCard({ cardId: "#10", tags: ["tutorial", "organization", "feature:tags"] })` still works in v4.2 with auto-create + slugify normalization. Response includes `_deprecated` and any near-miss `_didYouMean` hints. Removed in v5.0.0.',
				"**Outcome:** The tag combobox now shows the canonical labels for the project. Filtering by `bug` matches every card with that slug regardless of original casing.",
			].join("\n\n"),
			column: "Backlog",
			priority: "LOW",
		...
		},
		{
			...
			description: [
				"**What:** A milestone is a **release horizon** — a bounded set of cards intended to ship as a coherent unit. v4.2 settled the definition: see AGENTS.md.",
				"**Why it matters:** Milestones answer *when will this ship?* — agents use `getMilestoneContext` to report progress without re-counting cards manually. Naming convention: `vN.M.P — Theme` for software releases; free-form for non-release work, but each must answer 'what's the unit of completion?' in the description.",
				...
				"**Outcome:** You can report milestone status in one call. The roadmap view groups by horizon (Now / Next / Later / Done).",
			].join("\n\n"),
			column: "Backlog",
			priority: "MEDIUM",
