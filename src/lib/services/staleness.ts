/**
 * Shared staleness-check logic.
 *
 * Both the Next.js web server (`brief-payload-service.ts`) and the MCP
 * process (`src/mcp/staleness.ts` thin re-export, `src/mcp/server.ts`,
 * `src/mcp/tools/session-tools.ts`) need the same staleness evaluation
 * over `Claim` rows. Each process owns its own `PrismaClient`, so this
 * module accepts `db: PrismaClient` as a parameter rather than importing
 * a singleton — mirrors the `src/lib/services/handoff.ts` pattern and
 * satisfies the v6.2 decision that `src/server/` and `src/mcp/` never
