...
registerExtendedTool("getCardContext", {
	...
	handler: (params) => safeExecute(async () => {
		const { boardId, cardId: cardRef, format } = params as { boardId: string; cardId: string; format: "json" | "toon" };
...
registerExtendedTool("getMilestoneContext", {
	...
	annotations: { readOnlyHint: true },
	handler: (params) => safeExecute(async () => {
		const { boardId, milestone, format } = params as { boardId: string; milestone: string; format: "json" | "toon" };
...
registerExtendedTool("getTagContext", {
	...
	annotations: { readOnlyHint: true },
	handler: (params) => safeExecute(async () => {
		const { boardId, tag, format } = params as { boardId: string; tag: string; format: "json" | "toon" };
