---
title: Concepts
description: The mental model under Pigeon — sessions, handoffs, the briefMe loop, the saveHandoff wrap-up, and the deprecation calendar. The why under the why.
---
...
Handoffs are written by `saveHandoff` and read by `briefMe`. They live in the `note` table with `kind: "handoff"`, are scoped per board, and roll forward — every new handoff becomes the next session's primer.
...
| When | What's gone |
|---|---|
| **v5.x today** | Dual-bin: `mcpServers.project-tracker` + `scripts/mcp-start.sh` still work alongside the new `mcpServers.pigeon` + `scripts/pigeon-start.sh`. Legacy paths emit a `_brandDeprecation` field nudging migration. The renamed wrap-up tool `saveHandoff` (formerly `endSession`) ships in v5.2; the old name still resolves and emits a `_deprecated` warning. |
| **v6.0** | Legacy aliases removed. `mcpServers.project-tracker` stops working; `mcp-start.sh` is deleted. The `endSession` alias is removed — only `saveHandoff` resolves. You must have completed the rename per [`MIGRATING-TO-PIGEON.md`](https://github.com/2nspired/pigeon/blob/main/docs/MIGRATING-TO-PIGEON.md). Run `npm run doctor` to verify before you upgrade. |
| **Permanent** | `tracker.db` filename, `tracker.md` filename, the stable MCP tool names (`briefMe`, `saveHandoff`, `createCard`, `updateCard`, `moveCard`, `addComment`, `getTools`, `runTool`), Prisma table names, the `tracker://` URI scheme. None of these change. |
...
- **Essential + catalog split.** The 10 always-on tools are tiny (`briefMe`, `saveHandoff`, `createCard`, `updateCard`, `moveCard`, `addComment`, `registerRepo`, `checkOnboarding`, `getTools`, `runTool`). The other 50+ tools are discoverable via `getTools` and called via `runTool`. Your agent's system prompt stays small.
...
- [The session loop](/workflow/) — how the briefMe → work → saveHandoff cycle runs in practice
