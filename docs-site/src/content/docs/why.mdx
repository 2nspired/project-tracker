...
- **10 essential tools** are always loaded. They cover the hot path: start a session, create a card, update it, move it, comment on it, save a handoff at the end.
- **50+ extended tools** live behind two meta-tools: `getTools` browses them by category, `runTool` executes them by name.

```mermaid
flowchart TB
	subgraph Essential[Always loaded — 10 tools]
		direction LR
		E1[briefMe]
		E2[saveHandoff]
		E3[createCard]
...
- **It's composable with git.** `saveHandoff` runs `syncGitActivity` by default, so commits referencing `#N` get linked to cards automatically. Your git history is the other half of the handoff. (Pass `syncGit: false` for a mid-session checkpoint that skips the sync.)
