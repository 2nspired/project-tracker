...
The tracker is built around one loop. If you get the loop right, everything else falls out of it.

```mermaid
sequenceDiagram
	actor You
	participant Agent
	participant Tracker as Tracker (MCP + SQLite)
	participant Next as Next chat

	You->>Agent: Start conversation
	Agent->>Tracker: briefMe()
	Tracker-->>Agent: handoff + diff + top work + blockers + pulse
	loop Working on cards
		Agent->>Tracker: moveCard / updateCard / addComment (with intent)
		Tracker-->>You: Board updates live via SSE
	end
	You->>Agent: We're wrapping up
	Agent->>Tracker: endSession({ summary, nextSteps, ... })
	Tracker->>Tracker: save handoff, link commits, report touched cards
	Tracker-->>Agent: resume prompt for next chat
	You->>Next: Paste resume prompt → new conversation
	Next->>Tracker: briefMe()
```
...
1. **`briefMe` at the start of every session**
	...
	One tool call. Returns a compact primer — roughly 300–500 tokens — with the last agent's handoff, a diff of what's changed since, the top three work-next candidates, active blockers, open decisions, and staleness warnings.
