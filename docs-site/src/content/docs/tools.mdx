...
| `session` | 4 | `listHandoffs`, `loadHandoff`, `recordTokenUsage`, `recordTokenUsageFromTranscript` |
...
| `end-session` | Superseded — returns a pointer to the `saveHandoff` essential tool (was `endSession` pre-v5.2). |
...
- Session handoffs are stored as `Note` rows with `kind=handoff`; `saveHandoff` writes them and `briefMe` reads the latest.
